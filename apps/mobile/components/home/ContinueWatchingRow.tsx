...
export function ContinueWatchingRow({ profileId }: Props) {
  const { data } = useQuery({
    ...
    queryFn: () => api.history.get(profileId).then((r) => {
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 12, gap: 8 }}>
        {data.map((item) => {
          ...
          return (
            ...
            >
              {/* Thumbnail */}
              <View style={{ width: 160, height: 90, borderRadius: 6, overflow: 'hidden', backgroundColor: Colors.darkGray }}>
                <Image
                ...
                {/* Progress bar */}
                <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 3, backgroundColor: 'rgba(255,255,255,0.3)' }}>
                  <View style={{ height: '100%', width: `${pct}%`, backgroundColor: Colors.red }} />
                </View>
              </View>

              <View style={{ paddingTop: 6 }}>
                <Text style={{ color: Colors.white, fontSize: 12, fontWeight: '600' }} numberOfLines={1}>
                  {item.content.title}
                </Text>
                {remaining !== null && (
