...
export default function WatchScreen() {
  ...
  if (isTVWithSeasons) {
    ...
    const episode = season?.episodes.find((e) => e.episodeNumber === currentEpisode) ?? season?.episodes[0]
    if (episode) {
  ...
  function getNextEpisode() {
    ...
    if (idx < season.episodes.length - 1) return { season: currentSeason, episode: season.episodes[idx + 1]!.episodeNumber }
    const nextSeason = seasons.find((s) => s.seasonNumber === currentSeason + 1)
    if (nextSeason?.episodes.length) return { season: nextSeason.seasonNumber, episode: nextSeason.episodes[0]!.episodeNumber }
    return null
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.black }}>
      ...
      {/* Episode panel */}
      {showEpisodes && (
        <View style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 280, backgroundColor: Colors.darkGray, borderLeftWidth: 1, borderLeftColor: Colors.mediumGray }}>
          <View style={{ padding: 16, borderBottomWidth: 1, borderBottomColor: Colors.mediumGray, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <Text style={{ color: Colors.white, fontWeight: '700', fontSize: 16 }}>{content.title as string}</Text>
          ...
          {seasons.length > 1 && (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 44 }} contentContainerStyle={{ padding: 8, flexDirection: 'row', gap: 8 }}>
              {seasons.map((s) => (
                <TouchableOpacity
                  key={s.id}
                  onPress={() => setActiveSeason(s.seasonNumber)}
                  style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, backgroundColor: activeSeason === s.seasonNumber ? Colors.red : Colors.mediumGray }}
                  activeOpacity={0.8}
                >
                  <Text style={{ color: Colors.white, fontSize: 12, fontWeight: '600' }}>S{s.seasonNumber}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          )}
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 8 }}>
            {currentSeasonData?.episodes.map((ep) => {
              ...
              return (
                <TouchableOpacity
                  key={ep.id}
                  onPress={() => { setCurrentSeason(activeSeason); setCurrentEpisode(ep.episodeNumber); setShowEpisodes(false) }}
                  style={{ flexDirection: 'row', gap: 10, padding: 8, borderRadius: 10, marginBottom: 4, backgroundColor: isActive ? `${Colors.red}30` : 'transparent', borderWidth: isActive ? 1 : 0, borderColor: isActive ? `${Colors.red}60` : 'transparent' }}
                  activeOpacity={0.7}
                >
                  {ep.stillPath ? (
                    <Image source={{ uri: ep.stillPath }} style={{ width: 80, height: 45, borderRadius: 4 }} resizeMode="cover" />
                  ) : (
                    <View style={{ width: 80, height: 45, borderRadius: 4, backgroundColor: Colors.mediumGray, justifyContent: 'center', alignItems: 'center' }}>
                      <Text style={{ color: Colors.lightGray, fontSize: 18 }}>▶</Text>
                    </View>
                  )}
                  <View style={{ flex: 1, justifyContent: 'center' }}>
                    <Text style={{ color: isActive ? Colors.red : Colors.white, fontSize: 12, fontWeight: '600' }} numberOfLines={2}>
                      {ep.episodeNumber}. {ep.title}
                    </Text>
                    {ep.runtime && <Text style={{ color: Colors.lightGray, fontSize: 11, marginTop: 2 }}>{ep.runtime}m</Text>}
                  </View>
                </TouchableOpacity>
              )
            })}
