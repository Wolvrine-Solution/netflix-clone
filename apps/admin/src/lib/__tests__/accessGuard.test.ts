...
describe('decideAccess', () => {
  it('always passes through API routes, even for unauthenticated users', () => {
    expect(decideAccess({ isLoggedIn: false, pathname: '/api/auth/session' })).toEqual({ type: 'next' })
  })

  it('always passes through API routes for authenticated users', () => {
    expect(decideAccess({ isLoggedIn: true, pathname: '/api/auth/session' })).toEqual({ type: 'next' })
  })

  it('redirects an unauthenticated user away from a protected page to /login', () => {
    expect(decideAccess({ isLoggedIn: false, pathname: '/content' })).toEqual({ type: 'redirect', to: '/login' })
  })

  it('redirects an unauthenticated user away from the root page to /login', () => {
    expect(decideAccess({ isLoggedIn: false, pathname: '/' })).toEqual({ type: 'redirect', to: '/login' })
  })
  ...
  it('redirects an authenticated user away from /login to the home page', () => {
    expect(decideAccess({ isLoggedIn: true, pathname: '/login' })).toEqual({ type: 'redirect', to: '/' })
  })
  ...
  it('treats API-route check as a prefix match, not an exact match', () => {
    expect(decideAccess({ isLoggedIn: false, pathname: '/api/admin/content/123' })).toEqual({ type: 'next' })
  })
